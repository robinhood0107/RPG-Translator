...
test('sprite text adapter suppresses original parent-run glyphs while translated overlay is active', () => {
  const index = {
    translate({ text }) {
      if (text === 'ABC') return '가나다';
      return null;
  ...
  assert.equal(runOverlay._rpgTranslatorSpriteTextParentRunOverlay, true);
  assert.equal(runOverlay.bitmap._lastDrawText, '가나다');
  assert.equal(runOverlay.renderable, true);
  ...
  assert.equal(b.renderable, true);
  assert.equal(c.renderable, true);
});

test('sprite text adapter restores original glyphs when parent-run render is rejected as stale', () => {
  const index = {
    translate({ text }) {
      if (text === 'ABC') return '가나다';
      return null;
  ...
  assert.equal(SpriteTextAdapter.install(root, staleTranslator), true);
  const parent = new root.Sprite(null);
  const makeGlyph = (text, x) => {
    const sprite = new root.Sprite({ width: 16, height: 24, _rpgTranslatorGlyphText: text });
    sprite.x = x;
  ...
  assert.equal(b.renderable, true);
  assert.equal(c.renderable, true);
  assert.equal(baseOrchestrator.diagnostics().render_rejected, 1);
});
...
test('sprite text adapter moves existing overlays when a translated sprite is reparented', () => {
  const index = {
    translate({ text }) {
      if (text === 'Sprite JP') return 'Sprite KO';
      return null;
  ...
  assert.equal(SpriteTextAdapter.install(root, orchestrator), true);
  const firstParent = new root.Sprite(null);
  const secondParent = new root.Sprite(null);
  const sprite = new root.Sprite({ width: 120, height: 32, _rpgTranslatorGlyphText: 'Sprite JP' });
  firstParent.addChild(sprite);
  sprite.update();

  const overlay = firstParent.children[1];
  assert.equal(overlay._rpgTranslatorSpriteTextOverlay, true);
  assert.equal(overlay.parent, firstParent);

  secondParent.addChild(sprite);

  assert.deepEqual(firstParent.children, []);
  assert.equal(secondParent.children[0], sprite);
  assert.equal(secondParent.children[1], overlay);
  assert.equal(overlay.parent, secondParent);
  assert.equal(orchestrator.diagnostics().active_items, 1);
  assert.equal(orchestrator.diagnostics().archived_items, 0);
});
